...
- **Offer accept logic**: Use `Offers::AcceptService` for full workflow with notifications. Direct `offer.accept!` available for basic functionality. Both use pessimistic locking (`request.lock!` / `Request.lock.find`) to prevent race conditions.
...
- **_logo.html.erb** - Animated SVG logo with gradient text
- **_bottom_nav.html.erb** - Mobile bottom navigation bar (Home, Requests, Create, Notifications, Profile)
- **_skeleton_card.html.erb** - Skeleton loader placeholder for async content
...
- **animate_controller.js** - Scroll-based fade-in animations using Intersection Observer
- **loading_controller.js** - Skeleton loader display during content loading
...
Six major UI enhancements implemented:

1. **Animated SVG Logo** (`app/views/shared/_logo.html.erb`):
   - Gradient text with animation effects
   - Reusable partial rendered in both layouts
   - Supports dark mode with color adjustments

2. **Colored Category Icons** (`app/helpers/icon_helper.rb`):
   - `category_icon(category)` helper generates colored badge icons
   - Each category has unique color scheme (blue, green, purple, etc.)
   - Used in request cards and filters

3. **Skeleton Loaders** (`app/views/shared/_skeleton_card.html.erb`):
   - Animated placeholder cards shown during loading
   - Controlled by `loading_controller.js` Stimulus controller
   - Provides visual feedback for async content loading

4. **Hero Section Enhancements** (`app/views/requests/index.html.erb`):
   - Background pattern with decorative elements
   - Floating animated shapes
   - Wave transition between sections
   - Gradient overlays with dark mode support

5. **Scroll-based Animations** (`app/javascript/controllers/animate_controller.js`):
   - Cards fade in as user scrolls
   - Uses Intersection Observer API
   - CSS classes: `animate-fade-in`, `animate-slide-up`
   - Configurable via `data-animate-*` attributes

6. **Mobile Bottom Navigation** (`app/views/shared/_bottom_nav.html.erb`):
   - Fixed bottom bar on mobile (< md breakpoint)
   - Quick access: Home, Requests, Create (+), Notifications, Profile
   - Floating "+" button for creating new requests
   - z-index: 50 (below mobile drawer at z-60)
   - Shows user avatar or default icon
   - Active state highlighting based on current controller

**Related files:**
- `public/dark-mode.css` - Additional dark mode styles and animations
- `app/views/layouts/application.html.erb` - Mobile drawer with z-[60] for proper layering
- Translations: `nav.home`, `nav.requests`, `nav.create`, `nav.notifications`, `nav.profile`

**OmniAuth Graceful Degradation:**
Devise views check for OmniAuth availability before rendering OAuth buttons:
```erb
<% if devise_mapping.omniauthable? && resource_class.omniauth_providers.any? %>
  <%# OAuth buttons here %>
<% end %>
```
This allows local development without Google OAuth credentials configured (no providers registered = no buttons).

## Security & Architecture Decisions

### OmniAuth
- Only POST requests allowed (`OmniAuth.config.allowed_request_methods = [:post]`) — GET is disabled to prevent CSRF
- CSP nonce uses `SecureRandom.base64(16)`, not session ID

### Concurrency / Race Conditions
- **Offer acceptance** — both `Offer#accept!` and `Offers::AcceptService` use pessimistic locking:
  - `Offer#accept!` calls `request.lock!` (SELECT FOR UPDATE on the associated record, keeps in-memory object in sync)
  - `AcceptService` calls `Request.lock.find(offer.request.id)` and re-validates inside the transaction
- **Request completion** — `Requests::CompleteService` uses `Request.lock.find(request.id)` inside transaction

### Service Objects
- `NotificationUrlBuilder` lives in `app/services/notification_url_builder.rb` (extracted from `notification.rb`)
- `ApplicationService#with_transaction` rescues only `ActiveRecord::RecordInvalid` and `ActiveRecord::RecordNotFound`, not `StandardError` — programming errors bubble up intentionally

### Admin Authorization
- `Admin::UsersController::ALLOWED_ROLES = %w[user admin moderator]` — role value is validated against this whitelist before any other checks in `authorize_role_change!`

### Performance Patterns
- Use subqueries instead of `pluck` for large sets: `where(notifiable_id: conversation.messages.select(:id))` not `where(notifiable_id: conversation.messages.pluck(:id))`
- Request index eager loads: `includes(:category, :user, offers: :user)` — nested to avoid N+1 on offer users

### Frontend Security
- Never use `innerHTML` with server-broadcast data — use `textContent` or DOM API (see `notifications_controller.js#showToast`)
- Never use inline `onclick` attributes — use `addEventListener` instead (CSP blocks inline handlers)

### Validations
- `Institution#website` validated with `WEBSITE_REGEXP = /\Ahttps?:\/\/.+\z/` — explicit, replaces unpredictable `URI::DEFAULT_PARSER.make_regexp`
- `InstitutionMember` has `validates :user_id, uniqueness: { scope: :institution_id }` — duplicate member adds are caught at model level

## Frontend / UX Conventions

### Stimulus Controllers
- `password_match_controller.js` — real-time password confirmation validation. Attach with `data-controller="password-match"`, targets: `password`, `confirmation`, `hint`. Pass `data-password-match-match-text` and `data-password-match-mismatch-text` for i18n. Hint `<p>` should have `aria-live="polite"`.
- `notifications_controller.js` — manages dropdown open/close and syncs `aria-expanded` on `buttonTarget`

### Accessibility Rules
- All decorative SVG icons must have `aria-hidden="true"`
- Interactive buttons that only contain icons must have `aria-label`
- Navigation items should have `aria-current="page"` when active
- Dropdowns need `aria-haspopup`, `aria-expanded`, `aria-controls` on the trigger button
- Dynamic hint/status text should use `aria-live="polite"` for screen reader announcements

### Navigation
- Bottom nav active state: use `controller_name` / `action_name` / `current_page?` — never `request.path.include?(...)`
- Admin sidebar active state: use `controller_name == 'resource_name'` — never `request.path.include?(...)`
- "Back to all requests" link: use `controller.request.referer` if it matches `/requests` index, fallback to `requests_path` — preserves filters

### Translations
- Sawab balance tooltip: `t('nav.sawab_tooltip')` defined in all 3 locales
- Password match feedback: `t('users.edit.passwords_match')` / `t('users.edit.passwords_mismatch')` in all 3 locales
- Filter button loading state: `t('filters.applying_filters')` used with `data-turbo-submits-with`

### Forms
- Submit buttons with side effects should use `data-turbo-submits-with` to show loading state and prevent double-submit
...
- `docs/ARCHITECTURE.md` - Security rules, concurrency patterns, performance guidelines
